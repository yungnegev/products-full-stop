...
export const increment = (id) => {
    ...
    price.innerHTML = spacesForThousands(Math.round(parseInt(counter.innerHTML) * priceValue));
    // discount
    const discount = document.getElementById(`d-${id}`);
    const noDiscount = discount.dataset.noDiscount;
    discount.innerHTML = `${spacesForThousands(Math.round(parseInt(counter.innerHTML) * noDiscount))} сом`
