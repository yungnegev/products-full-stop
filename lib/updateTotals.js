import { spacesForThousands, removeSpaces } from './utils/utils.js';
...
export const updateTotals = () => {
    ...
    productPrices.forEach((price, index) => {
        if (cartItemsCheckboxes[index].value === 'on') {
            totalPriceValue += parseInt(removeSpaces(price.innerHTML));

        }
    })
    productQuantities.forEach((quantityValue, index) => {
        if (cartItemsCheckboxes[index].value === 'on') {
            quantity += parseInt(removeSpaces(quantityValue.innerHTML));
        }
    })

    noDiscountPrices.forEach((price, index) => {
        if (cartItemsCheckboxes[index].value === 'on') {
            totalNoDiscountValue += parseInt(removeSpaces(price.innerHTML));
        }
    })

    totalDiscountValue.innerHTML = `−${spacesForThousands(totalNoDiscountValue - totalPriceValue)} сом`
    totalNoDiscount.innerHTML = `${spacesForThousands(totalNoDiscountValue)} сом`
    totalPrice.innerHTML = spacesForThousands(totalPriceValue);
